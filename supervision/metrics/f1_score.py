    ...
    def _compute_confusion_matrix(
        ...
        for class_idx, class_id in enumerate(unique_classes):
            ...
            if num_predictions == 0:
                ...
                confusion_matrix[class_idx, :, 1] = num_predictions
            else:
                matches = sorted_matches[is_class]
                confusion_matrix[class_idx, :, 0] = matches.sum(axis=0)
                confusion_matrix[class_idx, :, 1] = (1 - matches).sum(axis=0)
                confusion_matrix[class_idx, :, 2] = num_true - matches.sum(axis=0)
